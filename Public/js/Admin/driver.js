$(function(){
    ...
    //判断在职还是离职状态，并设置radio选中状态

    if($('#the_driver_is_active').val() == '1'){
        $("input:radio[name='driver_is_active']").eq(0).attr("checked",true);

        $('#leave_date').hide();

    }else if ($('#the_driver_is_active').val() == '0'){
        $("input:radio[name='driver_is_active']").eq(1).attr("checked",true);
        $('#leave_date').show();
    }

    $('input:radio[name="driver_is_active"]').change( function(){
        var val=$('input:radio[name="driver_is_active"]:checked').val();
        if(val== '1'){
            //隐藏的时候先情况离职日期
            $('#driver_leave_date').val('');
            $('#leave_date').hide();
        }else{
            $('#leave_date').show();
        }
    ...
    if($('#updateForm').length > 0) {
        ...
        $('form :input').blur(function(){
            ...
            //车主验证
            if( $(this).is('#driver_name') ){
                var idname = 'driver_name';
...
function delDriverInfo(id){
    ...
    $.ajax({
    url:ROOT+"/Admin/Driver/doAction/action/del"//改为你的动态页
    ,type:"POST"
    ,data:{
            'id':id
        }
    ...
    ,success:function(json){
        if(json.success == 1){
            alert('删除成功');
            location = ROOT+"/Admin/Driver/doAction/action/all";
